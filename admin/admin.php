...
add_action('wp_ajax_emseo_save_page',function(){check_ajax_referer('emseo_pages','nonce');$id=absint($_POST['post_id']??0);$raw=json_decode(wp_unslash($_POST['fields']??'{}'),true);if(!emseo_save_post_fields($id,$raw))wp_send_json_error(array('message'=>'保存できません。'));wp_send_json_success(array('message'=>'ページSEOを保存しました。'));});
...
function emseo_field($key,$label,$type='text',$help=''){$s=emseo_settings();$value=$s[$key]??'';echo '<label class="emseo-field"><span>'.$label.'</span>';if($type==='textarea')echo '<textarea name="emseo_settings['.esc_attr($key).']" rows="3">'.esc_textarea($value).'</textarea>';else echo '<input type="'.esc_attr($type).'" name="emseo_settings['.esc_attr($key).']" value="'.esc_attr($value).'">';if($help)echo '<small>'.$help.'</small>';echo '</label>';}
function emseo_check($key,$label){$s=emseo_settings();echo '<label class="emseo-check"><input type="hidden" name="emseo_settings['.esc_attr($key).']" value="0"><input type="checkbox" name="emseo_settings['.esc_attr($key).']" value="1" '.checked($s[$key]??'0','1',false).'><span>'.$label.'</span></label>';}
